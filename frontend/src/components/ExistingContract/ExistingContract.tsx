...
const ExistingContract: FC<ContractProp> = ({
  arbiter,
  contractAddress,
  beneficiary,
  amount,
  ...
  return (
    <div className="flex flex-col flex-1 gap-2 bg-gray-200 p-4 font-serif">
      <p className="text-black font-bold">
        Arbiter: <span className="font-[100] text-sm">{arbiter}</span>
      </p>
      <p className="font-bold text-black">
        Beneficiary: <span className="font-[100] text-sm">{beneficiary}</span>
      </p>
      <p className="font-bold text-black">
        View on Etherscan:{" "}
        <a
      ...
      {isSubmitting ? (
        ...
        <button
          ...
          className={`${approved ? "bg-[#5aa75a]" : "btn"}`}
          disabled={approved}
        >
